...
resource "azurecaf_name" "dmz_to_clz" {
  resource_type = "azurerm_virtual_network_peering"
  prefixes      = var.global_settings.azurecaf_name.prefixes
  suffixes      = ["clz"]
}

resource "azurecaf_name" "clz_to_dmz" {
  resource_type = "azurerm_virtual_network_peering"
  prefixes      = var.global_settings.azurecaf_name.prefixes
  suffixes      = ["dmz"]
}
...
resource "azurerm_virtual_network_peering" "dmz_to_clz" {
  name                      = azurecaf_name.dmz_to_clz.result
  resource_group_name       = var.resource_group.name
  virtual_network_name      = azurerm_virtual_network.this.name
...
resource "azurerm_virtual_network_peering" "clz_to_dmz" {
  provider = azurerm.connectivity_landing_zone

  name                      = azurecaf_name.clz_to_dmz.result
  ...
  depends_on = [azurerm_virtual_network_peering.dmz_to_clz]
}
