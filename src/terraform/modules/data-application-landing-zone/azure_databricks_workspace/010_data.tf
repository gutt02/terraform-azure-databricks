locals {
  ...
  subscription_id                                                  = regex(local.resource_regex_subnet, var.private_endpoints_subnet_id)[0]
  resource_group_name                                              = regex(local.resource_regex_subnet, var.private_endpoints_subnet_id)[1]
  ...
  databricks_public_subnet_name                                    = regex(local.resource_regex_subnet, var.databricks_public_subnet_id)[3]
  databricks_private_subnet_name                                   = regex(local.resource_regex_subnet, var.databricks_private_subnet_id)[3]
  private_dns_zone_azure_databricks_name                           = regex(local.resource_regex_private_dns_zone, var.private_dns_zone_azuredatabricks_id)[2]
  connectivity_landing_zone_subscription_id                        = regex(local.resource_regex_private_dns_zone, var.connectivity_landing_zone_private_dns_zone_azuredatabricks_id)[0]
  connectivity_landing_zone_resource_group_name                    = regex(local.resource_regex_private_dns_zone, var.connectivity_landing_zone_private_dns_zone_azuredatabricks_id)[1]
...
data "azurerm_private_dns_zone" "databricks" {
  ...
  name                = local.private_dns_zone_azure_databricks_name
  resource_group_name = local.resource_group_name
}

data "azurerm_private_dns_zone" "connectivity_landing_zone_databricks" {
  provider = azurerm.connectivity_landing_zone_subscription
