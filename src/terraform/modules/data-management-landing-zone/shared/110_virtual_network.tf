...
resource "azurecaf_name" "dmlz_to_clz" {
  resource_type = "azurerm_virtual_network_peering"
  prefixes      = var.global_settings.azurecaf_name.prefixes
  suffixes      = ["clz"]
}

resource "azurecaf_name" "clz_to_dmlz" {
  resource_type = "azurerm_virtual_network_peering"
  prefixes      = var.global_settings.azurecaf_name.prefixes
  suffixes      = ["dmlz"]
}
...
resource "azurerm_virtual_network_peering" "dmlz_to_clz" {
  name                      = azurecaf_name.dmlz_to_clz.result
  resource_group_name       = azurerm_resource_group.this.name
  virtual_network_name      = azurerm_virtual_network.this.name
...
resource "azurerm_virtual_network_peering" "clz_to_dmlz" {
  provider = azurerm.connectivity_landing_zone_subscription

  name                      = azurecaf_name.clz_to_dmlz.result
  ...
  depends_on = [
    azurerm_virtual_network_peering.dmlz_to_clz
  ]
}
