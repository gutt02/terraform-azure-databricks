locals {
  ...
  schema_privileges = flatten([
    for grant_key, grant in var.databricks_schema.grants : [
      ...
      } if object_privilege.type == "schema"
    ]
  ])
...
resource "time_sleep" "delay_databricks_grants_schema" {
...
resource "time_sleep" "delay_databricks_grants_storage_credential" {
  count = length(local.storage_credential_privileges) > 0 ? length(databricks_storage_credential.this) : 0

  depends_on = [databricks_grants.schema]

  create_duration = local.default_databricks_api_delay
}

resource "time_sleep" "delay_databricks_grants_external_location" {
  count = length(local.external_location_privileges) > 0 ? length(databricks_external_location.this) : 0
...
resource "databricks_grants" "schema" {
  ...
  depends_on = [time_sleep.delay_databricks_grants_schema]
}
